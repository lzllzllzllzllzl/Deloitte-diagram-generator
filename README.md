...
### v23 — 活动编号可编辑 + API Key 验证 ✅
- **新增**：活动编号可编辑
  - 每个活动标题处添加可编辑的编号输入框
  - 编号显示为绿色边框的输入框，可直接点击修改
  - 新增活动时自动分配下一个编号（全局最大编号+1）
  - 删除活动时不再重新编号，保留用户自定义的编号
  - 修改编号后自动同步到流程信息
- **修复**：层级指示器文字对齐
  - 调整 CSS，让"S 泳道 → A 活动 → D 文档"在同一行整齐对齐
- **验证**：API Key 功能确认正常工作
  - 左上角密码框输入或修改 API Key
  - 点击"🚀 AI 优化并生成 XML"时使用当前输入的 API Key
  - 发送到 MiMo API 进行身份验证
- **状态**：已完成### 进行中（v24）
- 🔧 待确认下一步需求
...
当 AI 生成的 Draw.io XML 中活动编号、名称、泳道分配与用户输入不一致时（如编号被重新生成、活动标题被忽略），通常是**提示词过载**导致。

### 根因分析

当前提示词结构：
- 任务描述（约 200 字）
- 流程信息（用户输入，约 500-800 字）← 核心信息
- 德勤标准约束（约 3000+ 字）← 规范占据大量篇幅

**3000+ 字的规范会稀释 500 字的流程信息**，导致：
- 活动标题被忽略
- 编号被重新生成（AI 倾向于"优化"）
- 文档/系统信息丢失
- AI 的"创造性"覆盖了用户的具体指令
...
在 `genWithAI()` 函数中增加优先级声明，将流程信息放在最高优先级位置：

- 活动编号、名称、泳道分配、文档信息必须完全按照流程信息执行
- 设计规范作为"参考"而非"覆盖"

#### 方案二：两阶段生成（推荐）

1. **第一阶段**：验证和规范化流程信息，提取结构化数据（JSON）
2. **第二阶段**：基于结构化数据生成 XML

#### 方案三：分离规范到 System Message（最优）

将德勤规范放入 System Message，流程信息放入 User Message，降低 temperature 到 0.3：
...
body: JSON.stringify({
  ...
  messages: [
    {
      role: 'system',
      content: '你是严格的XML生成器。用户提供的编号、名称、配置是最高优先级，必须原样使用。'
    },
    {
      role: 'user',
      content: fullPrompt
...
1. **降低 temperature**（0.3）→ 减少 AI 的创造性修改
2. **明确优先级** → 流程信息 > 设计规范
3. **分离约束** → System Message 放规范，User Message 放流程
4. **精简规范** → 只保留关键规则，而非全部 3000 字
5. **在流程信息中强调编号** → 明确标注"编号已固定，禁止修改"
6. **添加验证逻辑** → AI 返回后检查编号是否与流程信息一致
